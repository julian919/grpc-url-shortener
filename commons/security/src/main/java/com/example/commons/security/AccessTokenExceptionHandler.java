 ...
 * once" logic is unaffected. The reason only adds detail.
 *
 * <p>Lives in the shared platform module: every service that verifies a token needs exactly this.
 */
public class AccessTokenExceptionHandler implements GrpcExceptionHandler {

  /** Same domain auth-service uses, so each (reason, domain) pair is one error everywhere. */
